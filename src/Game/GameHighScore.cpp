...
bool GameHighScore::paint(Image& backBuffer)
{
	...
	TextFont.PrintCenter(backBuffer,150+xco+shadow,"HALL OF FAME",IMAGE_BLTMODE_HALF,-1,0,0,xco2);
	TextFont.PrintCenter(backBuffer,150+xco,"HALL OF FAME",IMAGE_BLTMODE_TRANS,-1,0,0,xco2);
	for(int x=0;x<HIGH_SCORE_ENTRIES; x++)
	{
		...
		char tmp[100];
		TextFont.PrintAt(backBuffer,shadow+xco+304-16*strNames[x].length(),shadow+xOff+xco2+180+29*x,strNames[x].c_str(),IMAGE_BLTMODE_HALF);
		TextFont.PrintAt(backBuffer,xco+304-16*strNames[x].length(),xOff+xco2+180+29*x,strNames[x].c_str(),IMAGE_BLTMODE_TRANS);
		sprintf(tmp," %d",iPoints[x]);
		TextFont.PrintAt(backBuffer,shadow+xco+320,shadow+xOff+xco2+180+29*x,tmp,IMAGE_BLTMODE_HALF);
		TextFont.PrintAt(backBuffer,xco+320,xOff+xco2+180+29*x,tmp,IMAGE_BLTMODE_TRANS);
	}
		alpha+=.08;
	return true;
}
